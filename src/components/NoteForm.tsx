...
import ToastEditor from "./ToastEditor";
import Dragger from "antd/es/upload/Dragger";
import { CloudUpload, ChevronUp, Plus } from "lucide-react"; // Added Icons
import CertificateDocumentSceoundFixViewer from "./CertificateDocumentSceoundFixViewer";
...
const NoteForm: React.FC<NoteFormProps> = ({
  ...
  // ✅ State to manage visibility of Extra Cost Description
  ...
  useEffect(() => {
    if (initialData) {
      ...
      // ✅ Auto-expand if description exists
      if (initialData.description?.trim()) {
        setShowExtraDesc(true);
      }
    }
  }, [initialData, reset]);
  ...
  return (
    ...
    >
      ...
      {/* Client + Admin Comments (Hidden if pending) */}
      {isClientCommentVisible && (
        <>
          <div className="w-full px-4 flex flex-col gap-3">
            <label className="text-[#2B3738] text-sm font-medium">
        ...
        {mode === "edit" && (
          <>
            ...
            <Button type="primary" htmlType="submit" loading={updating}>
              {isApprovedOrRejected ? "Reply" : "Update Note"}
            </Button>
          </>
        )}
        {mode === "view" && (
