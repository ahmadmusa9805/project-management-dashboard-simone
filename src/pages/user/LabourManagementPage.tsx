...
const LaborTable: React.FC = () => {
  ...
  const { data, isLoading, refetch } = useGetAllLaboursQuery({
    page,
  ...
  // Refetch on debounced search or page change
  useEffect(() => {
    refetch();
  }, [debouncedSearchText, page, refetch]);
  ...
  return (
    <>
      <div className="bg-white min-h-screen mx-auto w-full p-4">
        <div className="flex justify-between mb-4">
          <h1 className="text-2xl font-semibold">Labors</h1>
          <CustomSearchInput
            onSearch={(val) => {
              setSearchText(val);
              setPage(1);
        ...
        {!isLoading && laborData.length === 0 && (
          <p className="text-center py-6 text-gray-500">No labor found.</p>
        )}
        ...
        {!isLoading && laborData.length > 0 && (
          <>
            ...
            {/* Pagination */}
            {!isLoading && data?.meta?.totalPage && data.meta.totalPage > 1 && (
              <CustomPagination
                page={page}
